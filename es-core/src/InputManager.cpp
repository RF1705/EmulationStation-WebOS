...
bool InputManager::parseEvent(const SDL_Event& ev, Window* window)
{
...

	{
		...
		// 0 / red / Back -> B (Escape)
		if(ev.key.keysym.scancode == SDL_WEBOS_SCANCODE_BACK || ev.key.keysym.scancode == SDL_SCANCODE_AC_BACK || mappedKey == SDLK_AC_BACK || mappedKey == 461)
			mappedKey = SDLK_ESCAPE;
		// 1 / green -> Start (F1), opens the EmulationStation menu
		else if(ev.key.keysym.scancode == SDL_WEBOS_SCANCODE_GREEN || ev.key.keysym.scancode == SDL_SCANCODE_WEBOS_HOME || ev.key.keysym.scancode == SDL_SCANCODE_MENU)
			mappedKey = SDLK_F1;
		// 2 / yellow -> Select (F2)
		else if(mappedKey == SDLK_2 || ev.key.keysym.scancode == SDL_WEBOS_SCANCODE_YELLOW)
			mappedKey = SDLK_F2;
#endif
